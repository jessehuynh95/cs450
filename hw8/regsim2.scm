...
(define (make-new-machine)
    (let ((pc (make-register))
      ...
      (let ((the-ops
        ...
        (define (execute-breakpoint) ;;; modified execute that checks for a breakppoint before execution.
          (let ((insts (get-contents pc)))
            (if (null? insts)
                'done
                (begin
                  (let ((break-here
                         (filter bps (lambda (breakpoint)
                                               (= (car (cdr (cdr breakpoint))) (car (cdr (cdr (car insts)))))))))
                    (if (null? break-here)
                        (execute) ;;;jumps to the normal execute from the original regsim
                        (let ((breakpoint (car break-here)))
                              (display "Breakpoint: " (car breakpoint) ", offset " (car (cdr breakpoint)))
                              (newline))))))))                  
        (define (execute) ;;;normal execute from the original regsim, we get here after checking for breakpoints
          (let ((insts (get-contents pc)))
            (if (null? insts)
                'done
                (begin
        ...
        (define (dispatch message)
          (cond ((eq? message 'start)
                ...
                ((eq? message 'install-labels)
                 (lambda (lab) (set! labs lab)))
                ;;; adds breakpoints by setting the breakpoint list to a new list made by cons-ing the new-bps procedure called with
                ;;; the label, position, and labels list with the breakpoints list
                ((eq? message 'add-bp)
                 (lambda (label pos)
                   (set! bps (cons (new-bps label pos labs) bps))))
                ;;; removes breakpoints by setting the breakpoint list to a call of remove-val on the list of breakpoints
                ((eq? message 'remove-bp)
                 (lambda (label pos)
                   (let ((breakpoints (new-bps label pos labs)))
                     (if (member breakpoints bps)
                         (set! bps (remove-val breakpoints bps))))))
                ;;; removes all breakpoints by setting the breakpoints list to the empty list
                ((eq? message 'clear-bps)
                 (lambda () (set! bps '())))
                ;;; continues execution after a breakpoint by calling the original execut procedure
                ((eq? message 'proceed)
                 (lambda () (execute)))                
...
;;; new-breakpoint uses the label in question, the position, and the labels list                
(define (new-bps label pos labs)	
  (let ((lab (assoc label labs)))	
    (cond ((lab (list label pos (+ pos (car (cdr (cdr lab)))))))
          (else (error "no label, create breakpoint" label)))))

;;; filter from class slides/notes
...
;;; modified remove-val using filter
(define (remove-val val lst) (filter (lambda (vals) (not (eq? vals val))) lst))
...
(define (regsearch seq val)
  (cond
    ((null? seq) #f)
    ((eqv? (car (car seq)) val) #t)
    (else (regsearch (cdr seq) val))))

(define (extract-labels text receive)
  (define (labelcount text receive count)
    (if (null? text)
        ...
        (labelcount
         ...
         (lambda (insts labels)             ;; This is where labels and insts are
           (let ((next-inst (car text)))    ;;   accumulated (as on a stack):
             ...
             (if (symbol? next-inst)   ;; changed to a cond so I can run more than one procedure following it
                  (cond ((assoc next-inst  labellist)
                         ...
                         (receive insts                              ;; either like this
                                  (cons (make-label-entry next-inst  ;;  (it's a label)
                                                          insts count)
                                        labels)))
                        (else (receive (cons (make-instruction next-inst count) ;; or like this
                              insts)                       ;;  (it's an inst)
                        labels))))))
         (if (symbol? (car text))
         count
         (+ count 1)))))
  (labelcount text receive 0))
...
(define (set-breakpoint machine label pos)	
  ((machine 'add-bp) label pos)	
)	
(define (cancel-bp machine label pos)	
  ((machine 'remove-breakpoint) label pos)	
)	
