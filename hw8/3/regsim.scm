      ...
      (let ((the-ops
        ...
        (define (execute)
          (let ((insts (get-contents pc)))
            (cond ((not (null? insts))
                   (let ((current (filter breakpts (lambda (bp)
                                                     (eq? (car (cdr (cdr bp))) (car (cdr (cdr (car insts)))))))))
                     (cond ((not (null? current))
                            (let ((breakpt (car current)))
                              (display* "breakpoint: " (car breakpt) (car (cdr breakpt)))
                              (newline)))
                           (else (proceed)))))
                  (else 'done))))
        (define (proceed)
          ...
          (cond ((not (null? insts))
                ...
                (else 'done))))
        ;;;filter procedure
        (define (filter lst instr)
          ;;;first checks to make sure the list isnt null
          (cond ((not (null? lst))
                 (cond ((instr (car lst))
                        ;;;if true, cons the car of the list to a recursive call of filter on the rest of the list
                        (cons (car lst) (filter (cdr lst) instr)))
                       ;;;recursive call on the cdr of the list to continue looking for the instruction
                       (else (filter (cdr lst) instr))))
                (else '())))
        ;;;breakpoint cancel procedure using previously defined filter procedure
        (define (rmbp bp val)
          (filter val (lambda (v) (not (eq? v bp)))))
        ;;;function to install breakpoint if possible
        (define (installbp label pos labels)
          (cond ((assoc label labels)
                 (list (label pos (+ pos (car (cdr (cdr (assoc label labels))))))))
                (else (error "specified label not found, create breakpoint" label))))
        (define (dispatch message)
          (cond ((eq? message 'start)
                ...
                ;;;installing set-breakpoint
                ((eq? message 'set-breakpoint)
                 (lambda (label pos)
                   (set! breakpts (cons (installbp label pos labels) breakpts))))
                ;;;installing proceed, if we see proceed-machine we call our proceed procedure to continue to program and execute
                ((eq? message 'proceed-machine)
                 (lambda () (proceed)))
                ;;;installing cancel-breakpoint by checking if the sepcific offset is located in the breakpts list and if so removing
                ;;;it using the rmbp procedure
                ((eq? message 'cancel-breakpoint)
                 (lambda (label pos)
                   (let ((offset (installbp label pos labels)))
                     (if (member offset breakpts)
                         (set! breakpts (rmbp offset breakpts))))))
                ;;;one of the simpler ones, if we see 'cancel-all-breakpoints we set the breakpoint list to empty
                ((eq? message 'cancel-all-breakpoints)
                 (lambda () (set! breakpts '())))
                ((eq? message 'save-labels)
                (lambda (label) (set! labels label)))
                (else (error "Unknown request -- MACHINE " message))))
        dispatch)))

	(define (excounter count text receive)	
          (define (ecounter label-name insts counter)	
            (list label-name insts counter))	
          (define (icounter text counter)	
            (list text '() counter))	
          (cond ((not (null? text))	
                 (excounter (cond ((symbol? (car text)) 	
                                            count)	
                                           (else (+ 1 count)))	
                            (cdr text) 	
                            (lambda (insts labels) 	
                              (cond ((symbol? (car text))		
                                     (cond ((assoc (car text) labels)	
                                            (error "Two labels with the same name" (car text)))	
                                           (else (receive insts (cons (ecounter (car text) insts count) labels)))))	
                                    (else (receive (cons (icounter (car text) count)	
                                               insts) labels))))))
                (else (receive '() '()))))
...
(define (assemble controller-text machine)
  (extract-labels controller-text
                  (lambda (insts labels)
                    ...
                    insts)))
(define (exlabels text recieve) 	
  (excounter 0 text recieve) 	
  )
