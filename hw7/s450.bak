...
(define (xeval exprs env)
  (let ((action (lookup-action (type-of exprs))))
    (if action
        ;;; checking for lambda procedures
        (if (equal? action make-procedure)
            (action (lambda-parameters exprs)
        ...
        (cond ((self-evaluating? exprs) exprs)
	       ...
	       (if (lookup-action exprs)
              ...
              ;;; checks if expression is a thunk
              ((thunk? exprs) (list 'thunk (thunkexpr exprs)))
              ;;; added a condition to check if it is a primitive procedure or delayed
              ((application? exprs)
               (let ((proced (xeval (operator  exprs) env)))
                 (xapply proced
                         (if (primitive-procedure? proced)
                             (list-of-values (operands exprs) env)
                             (if (list? proced)
                                 (list-of-values-delayed (procedure-parameters proced) (operands exprs) env)
                                 (list-of-values-delayed proced (operands exprs) env))))))
              ;;; checks if we have <EOF>, and if so,
              ((eof-object? exprs) (exit))
	      (else
	       (newerror "Unknown expression type -- XEVAL " exprs))))))
...
(define (xapply procedure arguments)
  (cond ((primitive-procedure? procedure)
        ...
        ((user-defined-procedure? procedure)
         ;;; let to allow  multiple procedures during cond
         (let ((dothis
                (eval-sequence
                 (procedure-body procedure)
                 (xtend-environment
                  (procedure-untag procedure)
                  arguments
                  (procedure-environment procedure)))))
           ;;; Pops the recently added frame off of the-dynamic-environment
  ...
  (if (no-operands? exprs)
      ...
      (cons
       (let ((param2 (car param))
             (operand (first-operand exprs)))
         (cond ((delayed? param2) (thunkify operand env))
               ((dynamic? param2) (xeval operand the-dynamic-environment))
               ((reference? param2)
                (if (symbol? (first-operand exprs))
                    (referencify operand env)
                    (newerror "Reference must have defined symbol")))
               (else (xeval operand env)))
         )
       (list-of-values-delayed
        (cdr param) (rest-operands exprs) env))))
...
(define (install-special-form specform body)
  (cond ((defined? specform the-global-environment)
         (newerror "giving a defined name to a new special form: " specform))
        ((not (equal? #f
                      (let ((entry (assoc specform (cdr specform-table))))
                        (if entry (cdr entry) #f))))
         (newerror "reinstalling a special form: " specform))
      ...
      (let ((first (car clauses))
        ...
        (if (cond-else-clause? first)
            (if (null? rest)
                ...
                (newerror "ELSE clause isn't last -- COND->IF "
                       clauses))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;;	 Homework 7: Delayed, dynamic, reference arguments
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;; returns if the argument is tagged as a dynamic argument
(define (dynamic? expr)
  (tagged-list? expr 'dynamic))

;;; returns the variable of a dynamic argument
(define (dyn-var expr)
  (car (cdr expr)))
...
;;; returns whether or not the argument is tagged as a reference
(define (reference? expr)
  (tagged-list? expr 'reference))

;;; returns the variable of a reference argument
(define (refr-var expr)
  (car (cdr expr)))

;;; creates a thunk-like reference object
(define (referencify var env)
  (list 'reference var env))

;;; returns the env of the reference
(define (refrenv refr)
  (car (cdr (cdr refr))))

;;; force the evaluation of the reference
(define (refreval refr)
  (xeval (refr-var refr) (refrenv refr)))
...
(define (stream-cdr stream)
  (xeval (thunkexpr (cdr stream)) (thunkenv (cdr stream))))
...
;;; added a cond to differentiate between dynamic and delayed arguments
;;; added a cond for reference arguments as well
        ...
        (cons
         (let ((param2 (car param)))
           (cond ((delayed? param2) (del-var param2))
                 ((dynamic? param2) (dyn-var param2))
                 ((reference? param2) (refr-var param2))
                 (else param2))
           )
         (tagremove (cdr param)))
        )
...
;;; modified to also extend the dynamic environment first

(define (xtend-environment vars vals base-env)
  (if (= (length vars) (length vals))
      (let ((newframe (make-frame vars vals)))
        ;;; this pushes the new frame onto the dynamic environment
        (set! the-dynamic-environment (cons newframe the-dynamic-environment))
        ;;; then adds the frame onto the static environment
        (cons newframe base-env))
      
      (if (< (length vars) (length vals))
          (newerror "Too many arguments supplied " vars vals)
          (newerror "Too few arguments supplied " vars vals))))
            ...
            ((eq? var (car vars))
             (let ((val (car vals)))
               (cond ((thunk? val) (thunkeval  val))
                     ((reference? val) (refreval val))
                     (else val))))
            (else (scan (cdr vars) (cdr vals)))))
    (if (eq? env the-empty-environment)
        (newerror "Unbound variable " var)
      ...
      (cond ((null? vars)
            ...
            ((eq? var (car vars))
             (if (reference? (car vals))
                 (let ((refvar (refr-var (car vals))))
                   ;;; sets the reference var to val with the env from the object
                   (set-variable-value! refvar val (car (cdr (cdr (car vals))))
;                                        (if (eq? var refvar)
...
;                                            env)
            ...
            (else (scan (cdr vars) (cdr vals)))))
    (if (eq? env the-empty-environment)
        (newerror "Unbound variable -- SET! " var)
        (let ((frame (first-frame env)))
          (scan (frame-variables frame)
                (frame-values frame)))))
  (if (lookup-action var) (newerror "defining a special form name:" var)
  (env-loop env)))
...
;;; global variable for the dynamic environment
...
(define (install-primitive-procedure name action)
  (if (lookup-action name)
      (newerror "installing a primitive procedure with the name of a special form: " name)
...
(define (s450)
  ;;; you can jump here to restart after errors
  (call-with-current-continuation
   (lambda (return)
     (set! beginning return)))
  (if (not (eq? break 'yet))
  ...
  ;;; jump here to skip prompting and exit s450
  (call-with-current-continuation
   (lambda (quit)
     (set! break quit)))
  (if exit?
...
;;;sets exit? to #f and allows us to exit s450 via the call/cc
(define (exit)
  (set! exit? #f)
  (break '()))
...
;;; a new error to go with modified s450 that allows looping back
;;; to the s450 prompt
(define newerror
  (lambda errors
    (display "newError: ")
    (my-for-each display errors)
    (beginning '())))
...
(install-primitive-procedure 'eval-load eval-load)
...
;;; For this, we are using xeval on the first element of the stream as it is return
;;; the promise is a thunk which is stored for later and can be evaluated later.
(install-special-form 'cons-stream
                      (lambda (expr env)                        
                        (cons (xeval (car (cdr expr)) env) (thunkify (thunkenv expr) env))))
